<script>
		...
		const mappedSizes = sizeGroups.map((d, i) => {
			...
			if (d[1].length == 1) {
				if (i == sizeGroups.length - 1) {
					minSize =
						(+d[1][0].waist + +max(sizeGroups[i - 1][1], (d) => d.waist)) / 2;
					maxSize = +d[1][0].waist + 1;
				} else if (i == 0) {
					minSize = +d[1][0].waist - 1;
					maxSize =
						(+d[1][0].waist + +min(sizeGroups[i + 1][1], (d) => d.waist)) / 2;
				} else {
					minSize =
						(+d[1][0].waist + +max(sizeGroups[i - 1][1], (d) => d.waist)) / 2;
					maxSize =
				...
				minSize =
					(+d[1][0].waist + +max(sizeGroups[i - 1][1], (d) => d.waist)) / 2;
				maxSize =
					(+d[1][1].waist + +min(sizeGroups[i + 1][1], (d) => d.waist)) / 2;
			}
	...
	function generateAvatarImgs(pointsData) {
		// Added 'points' as an argument
	...
	let positionedAvatars = $derived.by(() => {
		if (!pointsData?.length || containerWidth === 0 || containerHeight === 0)
			return [];
	...
	$effect(() => {
		// Sets up axis
		if (containerWidth > 0) {
			selectAll("#beeswarm .x-axis").call(
				axisBottom(xScale)
		...
		const targetY =
			currentId <= 1 || (currentId == "to-enter" && introScroll)
				? height / 4
				: 1;
		const targetHeight =
			currentId <= 1 || (currentId == "to-enter" && introScroll)
		...
		if (currentSizeRanges) {
			...
			if (currentId == 1) {
				...
				highlightWidth = Math.max(
					0,
					xScale(currentSizeRanges[3].max) - xScale(currentSizeRanges[3].min)
				);
				...
				highlightWidth = Math.max(
					0,
					xScale(currentSizeRanges[6].max) - xScale(currentSizeRanges[6].min)
				);
			} else if (currentId === 10) {
				highlightStart = xScale(currentSizeRanges[8].min);
				highlightWidth = Math.max(
					0,
					xScale(currentSizeRanges[8].max) - xScale(currentSizeRanges[8].min)
				);
			} else if (currentId === 11) {
				highlightStart = xScale(currentSizeRanges[9].min);
				highlightWidth = Math.max(
					0,
					xScale(currentSizeRanges[9].max) - xScale(currentSizeRanges[9].min)
				);
			} else if (currentId === 12) {
				highlightStart = xScale(currentSizeRanges[10].min);
				highlightWidth = Math.max(
					0,
					xScale(currentSizeRanges[10].max) - xScale(currentSizeRanges[10].min)
				);
			} else if (currentId === 13) {
				highlightStart = xScale(currentSizeRanges[0].min);
				highlightWidth = Math.max(
					0,
					xScale(currentSizeRanges[9].max) - xScale(currentSizeRanges[0].min)
				);
			} else if (currentId >= 14) {
				highlightStart = xScale(currentSizeRanges[10].min);
				highlightWidth = Math.max(
					0,
					xScale(60) - xScale(currentSizeRanges[10].min)
				);
			} else {
...
<div
	...
	class:intro={introScroll}
	class:middle={!introScroll}
>
	<div class="sticky-container">
		<div class="visual-container">
			{#if (isNaN(currentId) || currentId == 0) && introScroll && currentId !== "exit"}
				<div
					transition:fade={{ duration: $reducedMotion ? 0 : 500 }}
					class="intro-title"
				>
			...
			{#if (currentId >= 2 && currentId < 7) || currentId >= 8}
				...
				>
					<p>{sizeLabel}</p>
					<p>
						Sizes: {filteredASTM && filteredASTM.length > 0
							? filteredASTM[0].sizeRange
							: ""}
					</p>
				</div>
			{/if}
				...
				<svg {width} {height}>
					{#if currentSizeRanges}
						...
						<g class="size-backgrounds">
							{#each currentSizeRanges as sizeRange, i}
								...
								<g
									...
									class:omit={omittedSizeFilters.includes(sizeRange.size) ||
										currentId < 1 ||
										currentId == "to-enter"}
									style="transition-delay: {$reducedMotion
										? 0
										: setDelay(i, scrollDir)}s"
								>
									<rect
										style="transition: {$reducedMotion
											? 'none'
											: 'all var(--ms-500) ease-in-out'}"
										{x}
										y={$animatedBand.y}
										width={rectWidth}
										height={$animatedBand.height}
										fill="#9ABBD9"
									/>
									<text
										style="transition: {$reducedMotion
											...
											: 'all var(--ms-500) ease-in-out'}"
										x={x + rectWidth / 2}
										y={currentId <= 1 || isNaN(currentId)
											? $animatedBand.height * 1.5
						...
						<g
							...
							style="transition: {$reducedMotion
								? 'none'
								: 'opacity var(--ms-500) ease-in-out'}"
							class:visible={currentId == 1 ||
								currentId == 6 ||
								currentId == 7 ||
								(currentId == "exit" && introScroll) ||
								(currentId == "to-enter" && !introScroll) ||
						...
						>
							<rect
								style="transition: {$reducedMotion
									? 'none'
									: 'all var(--ms-500) ease-in-out'}"
								x={$animatedHighlight.x}
								y={$animatedHighlight.y}
								width={$animatedHighlight.width}
								height={$animatedHighlight.height}
							/>
						</g>
					{/if}
	...
	<div class="scrolly-outer">
		<Scrolly bind:value>
			{#each filteredStages as stage}
				<div class="step step{stage.id} step-{stage.textPos}">
					{#if stage.text}
						<div class="text">
