<script>
    ...
    /*** SCROLLY ***/
    let value = $state(0);
    let { startStage = 0, endStage = null, introScroll = null, autoPlay = false, stageDuration = 2000 } = $props();
     let filteredStages = $derived(copy.socialScroll ? copy.socialScroll.slice(
          startStage, 
  ...
  '18'
    ...
    function playNextStage() {
        ...
        if (value < filteredStages.length - 1) {
  ...
  <div class="outer-container" class:intro={introScroll} class:middle={!introScroll}>
    <div class="sticky-container">
      <div class="visual-container">
        ...
        <!-- Static text box for auto-play mode -->
        {#if autoPlay && currentStage?.text}
          <div class="static-text-box" transition:fade={{duration: 300}}>
            <div class="text"
            style="opacity: 0"
            >
              <Text copy={currentStage.text} />
            </div>
          </div>
        {/if}
        ...
        <!-- {#if autoPlay}
          <div class="auto-play-controls">
            {#if !isPlaying}
              <button on:click={resetAutoPlay}>Play</button>
            {:else}
              <button on:click={stopAutoPlay}>Pause</button>
            {/if}
            <span class="stage-counter">{value + 1} / {filteredStages.length}</span>
          </div>
        {/if} -->
      </div>
    </div>
      ...
      <Scrolly bind:value>
        {#each filteredStages as stage}
          ...
          >
            ...
            {/if}
            <!-- <div class ="text">
                <p>By <span class=bold>age 15</span> most girls can wear women's clothing. The median teen's  wears a size <span class=orange-outline>Medium</span>.</p>
            </div> -->
          </div>
        {/each}
      </Scrolly>
  ...
  <style>
    ...
    .html-label {
      ...
      .auto-play-controls button {
          ...
          background: var(--ws-orange, #ff6b35);
          color: white;
          border: none;
          border-radius: 4px;
          font-family: var(--mono);
      ...
      .auto-play-controls button:hover {
          background: var(--ws-orange-dark, #e55a2b);
      }
